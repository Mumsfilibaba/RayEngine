...
namespace RayEngine
{
	namespace Graphics
	{
		...
		public:
			...
			virtual bool CreateDevice(IDevice** ppDevice, const DeviceInfo& deviceInfo) const = 0;
			//Create a swapchain for presenting images to the window
			virtual bool CreateSwapchain(ISwapchain** ppSwapchain, const SwapchainInfo& swapchainInfo) const = 0;
			//Create a device for interaction with the graphicscard and a swapchain for presenting images to the window
			virtual bool CreateDeviceAndSwapchain(IDevice** ppDevice, const DeviceInfo& deviceInfo,
				ISwapchain** ppSwapchain, const SwapchainInfo& swapchainInfo) const = 0;
			//Create a object for creation of shaders
			virtual bool CreateShaderCompiler(IShaderCompiler** ppCompiler) const = 0;
